...
function App() {
 ...
 <button onClick={() => setInstallOpen(true)}>영상 가져오기 (설치 안내 검증)</button>
 <LocalImportConnection installOpen={installOpen} onInstallClose={() => setInstallOpen(false)} key={String(offline)} connected={connected} disabled={false} onLoadCode={discover} onConnect={pair} onDisconnect={() => {void client.disconnect();setConnected(false);}}/></>
