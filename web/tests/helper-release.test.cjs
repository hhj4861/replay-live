...
test('only versioned first-party artifacts with checksums become install links', async () => {
  ...
  assert.equal((await load({ version: '0.2.0', development: true, downloads: [artifact] })).development, true);
  await assert.rejects(load({ version: '0.2.0', development: 'yes', downloads: [artifact] }));
