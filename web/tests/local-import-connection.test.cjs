...
test('unavailable helper has an error and retry uses automatic discovery', async () => {
  ...
  assert.match(text(env.render()), /설치 파일은 아직 배포 준비 중/);
...
test('only consent downloads the automatically selected package', async () => {
  ...
  const confirm = env.button('동의하고 설치 파일 다운로드'); assert.equal(confirm.props.disabled, false);
  ...
  assert.match(text(env.render()), /다운로드를 요청했습니다/); env.dispose();
});
test('unpublished package cannot download and file upload cancels pending pairing', async () => {
  ...
  assert.equal(env.button('동의하고 설치 파일 다운로드').props.disabled, true);
