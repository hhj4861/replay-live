...
DEFAULT_CLOUD_URL = 'https://replay-live-api.vercel.app'
...
def run_device_import(id, token, *, api, output, downloader, check_active, on_phase,
    ...
    def call(method, suffix, body=None):
        ...
        if response.status_code in (401, 403, 404, 409):
            raise SourceImportError('DEVICE_IMPORT_REVOKED')
    ...
    try:
        ...
        if (task.get('id') != id or not isinstance(task.get('max_bytes'), int)
            ...
            raise SourceImportError('DEVICE_IMPORT_INVALID')
        ...
        trusted = (destination.netloc == urlsplit(api).netloc or host.endswith('.blob.vercel-storage.com')
        ...
        code = error.code if isinstance(error, SourceImportError) else 'DEVICE_IMPORT_UPLOAD_FAILED'
        if not completed:
            try:
                client.post(endpoint + '/failure', headers=headers, json={'code': code}, timeout=10)
            except Exception:
                pass
        raise SourceImportError(code) from None
    finally:
